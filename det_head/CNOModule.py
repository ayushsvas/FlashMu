...
class ResidualBlock(nn.Module):
    def __init__(self,
        ...
        self.convolution2 = torch.nn.Conv2d(in_channels = self.channels, out_channels=self.channels, 
                                           kernel_size=self.conv_kernel, stride = 1, dilation=3,
                                           padding = 3*pad)
        self.convolution3 = torch.nn.Conv2d(in_channels = self.channels, out_channels=self.channels, 
                                           kernel_size=self.conv_kernel, stride = 1, dilation=9,
                                           padding = 9*pad)
        
        if self.batch_norm:
            self.batch_norm1  = nn.BatchNorm2d(self.channels)
            self.batch_norm2  = nn.BatchNorm2d(self.channels)
            self.batch_norm3  = nn.BatchNorm2d(self.channels)
